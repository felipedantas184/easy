...
export function DiscountCoupon({ storeId }: DiscountCouponProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Informação do Desconto no Resumo */}
      {state.discount?.applied && (
        <div className="border-t pt-3">
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">Subtotal</span>
            <span className="text-gray-900">R$ {state.total.toFixed(2)}</span>
          </div>
          
          <div className="flex justify-between text-sm text-green-600">
            <span>Desconto</span>
            <span>- R$ {state.discount.discountAmount.toFixed(2)}</span>
          </div>
          
          <div className="flex justify-between font-semibold border-t pt-2 mt-2">
            <span>Total com desconto</span>
            <span>R$ {(state.total - state.discount.discountAmount).toFixed(2)}</span>
          </div>
        </div>
      )}
    </div>
